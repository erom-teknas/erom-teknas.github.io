...
- Posts can embed an interactive flow diagram with `{% include flow.html id="<name>" %}`, drawn from `_data/flows/<name>.yml` (schema at the top of `_includes/flow.html`). Keep its content true to the post's text.
